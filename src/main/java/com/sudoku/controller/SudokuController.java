...
highlightErrorCell(r, c, true);
                    }else{
                            view.

highlightErrorCell(r, c, false);
                    }
                            }
                            }
...
updateStatus("Có lỗi trên bảng! (Các ô màu đỏ vi phạm luật)");
            }else{
                    if(logic.
...
updateStatus("Chúc mừng! Bạn đã giải xong Sudoku một cách hợp lệ!");
                }else{
                        view.

updateStatus("Bảng hợp lệ. Hãy tiếp tục giải!");
                }
                        }
                        });
...
updateStatus("Lỗi! Bảng hiện tại sai luật, không thể giải.");
                        view.
...
updateStatus("Đang giải bằng Backtracking...");
...
updateStatus("Đã giải xong bằng thuật toán Backtracking!");
            }else{
                    view.

updateStatus("Không tìm thấy giải pháp cho bảng này.");
            }
                    });
...
private void handleUserInput(int row, int col) {
        ...
        if (value != solution[row][col]) {
            ...
            if (isNewMistake) {
                view.updateMistakeUI(gameController.getMistakeCount(), gameController.getMaxMistakes());
    ...
    private void gameOver ( boolean won){
        ...
        if (won) {
            ...
            for (int i = 0; i < 9; i++)
                for (int j = 0; j < 9; j++)
                    if (currentBoard[i][j] == 0 || currentBoard[i][j] != solution[i][j])
                        return; // Còn ô chưa đúng → chưa thắng
